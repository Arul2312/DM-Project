...
def dti_loss(zD_pos, zT_pos, zD_neg, zT_neg):
    ...
    bce_loss = F.binary_cross_entropy(scores, labels)
    contrastive_loss = torch.mean(torch.relu(neg_score - pos_score + 1.0))  # Margin 1.0
    return bce_loss + 1.5 * contrastive_loss
...
def load_data(adjacency_matrix, feature_matrix, positive_samples, AY, batch_size=32, test_split=0.2):
    num_neg_samples = len(positive_samples) * 2
